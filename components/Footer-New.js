import Link from 'next/link';
import { useState } from 'react';
import { languages, quickLinks } from '../utilities/utilities';
import { FooterTextsConfig, FooterLinksConfig } from './FooterConfig';

const InitialState = {
  drop1: false,
  drop2: false,
  drop3: false,
  drop4: false,
};

export default function FooterNew() {
  const [dropdownState, setDropdownState] = useState(InitialState);

  const d = new Date();
  let year = d.getFullYear();

  function handleClick(dropdown) {
    setDropdownState((currentState) => ({
      ...currentState,
      [dropdown]: !currentState[dropdown],
    }));
  }
  return (
    <section id="footerNew">
      <div className=" hidden lg:!block bg-[#f3f5f7]">
        <div className="container-fluid !p-2 !text-start max-vw-100 !flex !flex-row !justify-between !flex-wrap !px-4 !pt-12 !pb-2 !mx-auto" style={{maxWidth:"1250px"}}>
          <div className="!flex !flex-col !justify-start !px-4">
            <h3 className="!ps-1 !pe-12 !me-12 !py-2 !mb-2">Courses</h3>
            {languages.map((language) => (
              <Link
                href={`/online-${language}-classes/`}
                className="!px-1 !py-2 font-normal text-base/6 hover:!underline hover:!font-black"
                style={{textDecoration:"none",lineHeight:"32px",color:"#5C5856"}}
                target="blank"
                key={language}
              >
                <span style={{ textTransform: 'capitalize' }}>{language}</span>
              </Link>
            ))}
          </div>
          <div className="!flex !flex-col !justify-start !px-4">
            <h3 className="!ps-1 !pe-12 !me-6 !py-2">Quick Links</h3>
            {quickLinks.map((quickLink) => (
              <Link
                href={quickLink.url}
                className="!px-1 !py-2 font-normal text-base/6 hover:!underline hover:!font-black"
                style={{textDecoration:"none",lineHeight:"32px",color:"#5C5856"}}
                key={quickLink.name}
              >
                {quickLink.name}
              </Link>
            ))}
          </div>
          <div className="!flex !flex-col !justify-start !px-4">
            <h3 className="!ps-1 !pe-12 !me-6 !py-2">Contact us</h3>            
            <Link className="!px-1 !py-2 font-normal text-base/6 hover:!underline hover:!font-black"
            style={{textDecoration:"none",lineHeight:"32px",color:"#5C5856"}}
             href="tel:+91-6366936953">
              +91-6366936953
            </Link>
            <Link className="!px-1 !py-2 font-normal text-base/6 hover:!underline hover:!font-black"
            style={{textDecoration:"none",lineHeight:"32px",color:"#5C5856"}}
            href="tel:+91-6366936559">
              +91-6366936559
            </Link>
            <Link className="!px-1 !py-2 font-normal text-base/6 hover:!underline hover:!font-black"
            style={{textDecoration:"none",lineHeight:"32px",color:"#5C5856"}}
              href="mailto:hello@bhasha.io"
            >
              hello@bhasha.io
            </Link>
            <Link
              className="!px-1 !py-2 font-normal text-base/6 hover:!underline hover:!font-black"
              style={{textDecoration:"none",lineHeight:"32px",color:"#5C5856"}}
              href="mailto:ops@bhasha.io"
            >
              ops@bhasha.io
            </Link>
          </div>
        </div>
        <div className="!flex !flex-row !justify-center !bg-[#f3f5f7] !pt-12">
          {/* <h3 className="ps-1 pe-4 py-2">Connect with us</h3> */}
            <Link
              href="https://www.facebook.com/bhashaApp/"
              className="!px-4 !py-2 footerNewLink hover:!underline hover:!font-black"
              key="Facebook"
              target="blank"
            >
            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" className="bi bi-facebook" viewBox="0 0 16 16">
              <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951"/>
            </svg>  
            </Link>
            <Link
              href="https://instagram.com/bhasha.io?utm_medium=copy_link"
              className="!px-4 !py-2 footerNewLink hover:!underline hover:!font-black"
              key="Instagram"
              target="blank"
            >
            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" className="bi bi-instagram" viewBox="0 0 16 16">
            <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.9 3.9 0 0 0-1.417.923A3.9 3.9 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.9 3.9 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.9 3.9 0 0 0-.923-1.417A3.9 3.9 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599s.453.546.598.92c.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.5 2.5 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.5 2.5 0 0 1-.92-.598 2.5 2.5 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233s.008-2.388.046-3.231c.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92s.546-.453.92-.598c.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92m-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217m0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334"/>
            </svg>
            </Link>
            <Link
              href="https://www.linkedin.com/company/bhasha-io"
              className="!px-4 !py-2 footerNewLink hover:!underline hover:!font-black"
              key="LinkedIn"
              target="blank"
            >
            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" className="bi bi-linkedin" viewBox="0 0 16 16">
            <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854zm4.943 12.248V6.169H2.542v7.225zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248S2.4 3.226 2.4 3.934c0 .694.521 1.248 1.327 1.248zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016l.016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225z"/>
            </svg>
            </Link>
            <Link
              href="https://www.youtube.com/channel/UCn4FevDRUbLVcmDrSinJ_eg"
              className="!px-4 !py-2 footerNewLink hover:!underline hover:!font-black"
              key="Youtube"
              target="blank"
            >
            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" className="bi bi-youtube" viewBox="0 0 16 16">
            <path d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104.022.26.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105-.009.104c-.05.572-.124 1.14-.235 1.558a2.01 2.01 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006-.087-.004-.171-.007-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.01 2.01 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31 31 0 0 1 0 7.68v-.123c.002-.215.01-.958.064-1.778l.007-.103.003-.052.008-.104.022-.26.01-.104c.048-.519.119-1.023.22-1.402a2.01 2.01 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007.172-.006.086-.003.171-.007A100 100 0 0 1 7.858 2zM6.4 5.209v4.818l4.157-2.408z"/>
            </svg>
            </Link>
          </div>
        <FooterTextsConfig />
        <div className="!bg-[#f3f5f7] text-[#5c5856] !text-base !py-6 !pt-12 !contactLink !px-12 !text-center">
            <span>Bhasha.io © Varta Labs Pvt. Ltd, {year}</span>
        </div>
      </div>
      
      { /* ---- Mobile View Footer ---- */ }

      <div className="!block lg:!hidden bg-[#f3f5f7]">
        <div
          className="!flex !justify-between !pt-6 !pb-2 !px-6 text-[#212529] font-medium"
          onClick={() => handleClick('drop1')}
          onKeyDown={() => handleClick('drop1')}
          role="button"
          tabIndex={0}
        >
          <span style={{ fontWeight: '500', fontSize: '14px' }}>Courses</span>
          {dropdownState.drop1 ? (
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
              fill="#4b2AAD"
              className="bi bi-chevron-up"
              viewBox="0 0 16 16"
            >
              <path
                fillRule="evenodd"
                d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"
              />
            </svg>
          ) : (
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
              fill="#4b2AAD"
              className="bi bi-chevron-down text-primary"
              viewBox="0 0 16 16"
            >
              <path
                fillRule="evenodd"
                d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708z"
              />
            </svg>
          )}
        </div>
        {languages.map((language) => (
          <Link
            href={`/online-${language}-classes`}
            className={
              dropdownState.drop1
                ? '!px-12 !py-1 !flex !flex-col hover:!underline hover:!font-black'
                : 'hidden'
            }
            style={{textDecoration:"none"}}
            target="blank"
            key={language}
          >
            <span
              style={{
                fontWeight: '400',
                fontSize: '12px',
                color: '#5C5856',
                textTransform: 'capitalize',
              }}
            >
              {language}
            </span>
          </Link>
        ))}

        <div
          className="!flex !justify-between !pt-2 !pb-2 !px-6 text-[#212529] font-medium"
          onClick={() => handleClick('drop2')}
          onKeyDown={() => handleClick('drop2')}
          role="button"
          tabIndex={0}
        >
          <span style={{ fontWeight: '500', fontSize: '14px' }}>
            Quick Links
          </span>
          {dropdownState.drop2 ? (
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
              fill="#4b2AAD"
              className="bi bi-chevron-up"
              viewBox="0 0 16 16"
            >
              <path
                fillRule="evenodd"
                d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"
              />
            </svg>
          ) : (
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
              fill="#4b2AAD"
              className="bi bi-chevron-down text-primary"
              viewBox="0 0 16 16"
            >
              <path
                fillRule="evenodd"
                d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708z"
              />
            </svg>
          )}
        </div>
        {quickLinks.map((quickLink) => (
          <Link
            href={quickLink.url}
            className={
              dropdownState.drop2
                ? '!px-12 !py-1 !flex !flex-col hover:!underline hover:!font-black'
                : 'hidden'
            }
            style={{textDecoration:"none"}}
            key={quickLink.name}
          >
            <span
              style={{ fontWeight: '400', fontSize: '12px', color: '#5C5856' }}
            >
              {quickLink.name}
            </span>
          </Link>
        ))}

        <div
          className="!flex !justify-between !pt-2 !pb-2 !px-6 text-[#212529] font-medium"
          onClick={() => handleClick('drop3')}
          onKeyDown={() => handleClick('drop3')}
          role="button"
          tabIndex={0}
        >
          <span style={{ fontWeight: '500', fontSize: '14px' }}>
            Connect with us
          </span>
          {dropdownState.drop3 ? (
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
              fill="#4b2AAD"
              className="bi bi-chevron-up"
              viewBox="0 0 16 16"
            >
              <path
                fillRule="evenodd"
                d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"
              />
            </svg>
          ) : (
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
              fill="#4b2AAD"
              className="bi bi-chevron-down text-primary"
              viewBox="0 0 16 16"
            >
              <path
                fillRule="evenodd"
                d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708z"
              />
            </svg>
          )}
        </div>
        <Link
          href="https://www.facebook.com/bhashaApp/"
          className={
            dropdownState.drop3
              ? '!px-12 !py-1 !flex !flex-col hover:!underline hover:!font-black'
              : 'hidden'
          }
          style={{textDecoration:"none"}}
          key="Facebook"
          target="blank"
        >
          <div className="d-flex align-items-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-facebook me-2" viewBox="0 0 16 16">
              <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951"/>
            </svg>
            <span
              style={{ fontWeight: '400', fontSize: '12px', color: '#5C5856' }}
            >
              Facebook
            </span>
          </div>
        </Link>
        <Link
          href="https://instagram.com/bhasha.io?utm_medium=copy_link"
          className={
            dropdownState.drop3
              ? '!px-12 !py-1 !flex !flex-col hover:!underline hover:!font-black'
              : 'hidden'
          }
          style={{textDecoration:"none"}}
          key="Instagram"
          target="blank"
        >
          <div className="d-flex align-items-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-instagram me-2" viewBox="0 0 16 16">
              <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.9 3.9 0 0 0-1.417.923A3.9 3.9 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.9 3.9 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.9 3.9 0 0 0-.923-1.417A3.9 3.9 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599s.453.546.598.92c.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.5 2.5 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.5 2.5 0 0 1-.92-.598 2.5 2.5 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233s.008-2.388.046-3.231c.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92s.546-.453.92-.598c.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92m-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217m0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334"/>
            </svg>
            <span
              style={{ fontWeight: '400', fontSize: '12px', color: '#5C5856' }}
            >
              Instagram
            </span>
          </div>
        </Link>
        <Link
          href="https://www.linkedin.com/company/bhasha-io"
          className={
            dropdownState.drop3
              ? '!px-12 !py-1 !flex !flex-col hover:!underline hover:!font-black'
              : 'hidden'
          }
          style={{textDecoration:"none"}}
          key="LinkedIn"
          target="blank"
        >
          <div className="d-flex align-items-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-linkedin me-2" viewBox="0 0 16 16">
              <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854zm4.943 12.248V6.169H2.542v7.225zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248S2.4 3.226 2.4 3.934c0 .694.521 1.248 1.327 1.248zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016l.016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225z"/>
            </svg>
            <span
              style={{ fontWeight: '400', fontSize: '12px', color: '#5C5856' }}
            >
              LinkedIn
            </span>
          </div>
        </Link>
        <Link
          href="https://www.youtube.com/channel/UCn4FevDRUbLVcmDrSinJ_eg"
          className={
            dropdownState.drop3
              ? '!px-12 !py-1 !flex !flex-col hover:!underline hover:!font-black'
              : 'hidden'
          }
          style={{textDecoration:"none"}}
          key="Youtube"
          target="blank"
        >
          <div className="d-flex align-items-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-youtube me-2" viewBox="0 0 16 16">
              <path d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104.022.26.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105-.009.104c-.05.572-.124 1.14-.235 1.558a2.01 2.01 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006-.087-.004-.171-.007-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.01 2.01 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31 31 0 0 1 0 7.68v-.123c.002-.215.01-.958.064-1.778l.007-.103.003-.052.008-.104.022-.26.01-.104c.048-.519.119-1.023.22-1.402a2.01 2.01 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007.172-.006.086-.003.171-.007A100 100 0 0 1 7.858 2zM6.4 5.209v4.818l4.157-2.408z"/>
            </svg>
            <span
              style={{ fontWeight: '400', fontSize: '12px', color: '#5C5856' }}
            >
              YouTube
            </span>
          </div>
        </Link>

        <div
          className="!flex !justify-between !pt-2 !pb-2 !px-6 text-[#212529] font-medium"
          onClick={() => handleClick('drop4')}
          onKeyDown={() => handleClick('drop4')}
          role="button"
          tabIndex={0}
        >
          <span style={{ fontWeight: '500', fontSize: '14px' }}>
            Contact Info
          </span>
          {dropdownState.drop4 ? (
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
              fill="currentColor"
              className="bi bi-chevron-up"
              viewBox="0 0 16 16"
            >
              <path
                fillRule="evenodd"
                d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"
              />
            </svg>
          ) : (
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
              fill="#4b2AAD"
              className="bi bi-chevron-down text-primary"
              viewBox="0 0 16 16"
            >
              <path
                fillRule="evenodd"
                d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708z"
              />
            </svg>
          )}
        </div>
        <div>
          <div
            className={
              dropdownState.drop4 ? '!px-12 !py-1 !flex !flex-col' : '!hidden'
            }
            style={{ fontWeight: '400', fontSize: '12px', color: '#5C5856' }}
          >
            Contact us:
            <Link
              className="!py-1 hover:!underline hover:!font-black"
              href="tel:+91-6366936952"
              style={{ fontWeight: '400', fontSize: '12px', color: '#5C5856',textDecoration:"none" }}
            >
              +91-6366936952
            </Link>
            <div
              className="!flex !flex-col !py-1"
              style={{ fontWeight: '400', fontSize: '12px', color: '#5C5856' }}
            >
              Email Us:
              <Link
                className=" !py-1 hover:!underline hover:!font-black"
                href="mailto:hello@bhasha.io"
                style={{
                  fontWeight: '400',
                  fontSize: '12px',
                  color: '#5C5856',
                  textDecoration:"none"
                }}
              >
                hello@bhasha.io
              </Link>
              <Link
                className=" !py-1 hover:!underline hover:!font-black"
                href="mailto:ops@bhasha.io"
                style={{
                  fontWeight: '400',
                  fontSize: '12px',
                  color: '#5C5856',
                  textDecoration:"none"
                }}
              >
                ops@bhasha.io
              </Link>
            </div>
            <div
              className="!flex !flex-col !py-1"
              style={{ fontWeight: '400', fontSize: '12px', color: '#5C5856' }}
            >
              Website:
              <Link
                className=" !py-1 hover:!underline hover:!font-black"
                href="/"
                style={{
                  fontWeight: '400',
                  fontSize: '12px',
                  color: '#5C5856',
                  textDecoration:"none"
                }}
              >
                https://www.bhasha.io/
              </Link>
            </div>
          </div>
        </div>
        <FooterTextsConfig />
        <div className="!pt-2 !mt-2 contactLink text-center bg-[#f3f5f7]">
          <span
            style={{ fontWeight: '400', fontSize: '12px', color: '#5C5856' }}
          >
            Bhasha.io © Varta Labs Pvt. Ltd, {year}
          </span>
        </div>
      </div>
    </section>
  );
}
